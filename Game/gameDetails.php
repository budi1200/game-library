<?php
    ...
    // include header
    $title = 'Game Library | Game Details';
    ...
    $sql_comments = $conn->prepare("SELECT c.id, c.user_id, c.comment, c.addedTime, u.username, i.url FROM comments c INNER JOIN users u ON u.id=c.user_id INNER JOIN images i ON u.image_id=i.id WHERE c.game_id=? ORDER BY c.addedTime DESC");
